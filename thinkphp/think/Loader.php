    ...
    public static function register($autoload = '')
    {
        ...
        if (is_dir(self::$composerPath)) {
            if (is_file(self::$composerPath . 'autoload_static.php')) {
                ...
                $declaredClass = get_declared_classes();    // 返回所有加载的类
                $composerClass = array_pop($declaredClass);     //最后一个是composer类
//                var_dump($declaredClass);
...
//                var_dump(self::$prefixLengthsPsr4);var_dump(self::$prefixDirsPsr4);die;
            } else {
                self::registerComposerLoader(self::$composerPath);
            }
        }

        // 注册命名空间定义(添加$prefixLengthsPsr4 和 $prefixDirsPsr4)
        self::addNamespace([
            'think'  => __DIR__,
...
//        var_dump($class);
        if (isset(self::$classAlias[$class])) {
            return class_alias(self::$classAlias[$class], $class);  // 类的别名设置  think\facade\Config => Config
        }

//        var_dump(self::findFile($class));die;
...
//        var_dump($logicalPathPsr4);   // think\Error.php
        $first = $class[0];
//        var_dump($first);     // 类名的第一个字符

        // think\composer think traits   app
        ...
        // 查找 PSR-4 fallback dirs    默认extend目录
